<?php
    ...
    $sqlQuery2 = $DB_Connection->prepare('SELECT * FROM `appointments` WHERE `Email`=?');
                      
    $sqlQuery2->execute(array($_SESSION["email"]));
...
$sql = 'UPDATE appointments SET Name=:temp_name,Age=:temp_age,Email=:temp_email,PhNo=:temp_phno,Gender=:temp_gender,Date=:temp_date,Time=:temp_time,Disease=:temp_disease,Doctor=:temp_doctor WHERE Email= :temp_email' ;
...
<html lang="en" dir="ltr">
  ...
  <div class="container-lg">
    ...
    <h1><i>Edit Your Appointment!</i></h1>
    <h3><u>Fill this Form to edit Appointment:</u></h3>
    ...
    <?php foreach($dataRecords2 as $appointments): ?>      <!--- the loop generates a row for each record --->
        ...
        <input type="text" value=<?= $appointments->Name; ?> placeholder="Enter Name" id="pname" name="txt_name"><br><br>
        
        <label for="page">Patient Age:</label>
        <input type="text" placeholder="Enter Age" id="page" value=<?= $appointments->Age; ?> name="combo_age"><br><br>
        <label for="pemail">Patient Email</label>
        <input type="email" placeholder="Enter Email" id="pemail" value=<?= $appointments->Email; ?> name="txt_email"><br><br>
        <label for="phone">Patient phone:</label><br><br>
        <input type="tel" id="phone" name="phone" placeholder="0336-xxxxxxx" value=<?= $appointments->PhNo; ?> pattern="[0-9]{4}-[0-9]{7}" required><br><br>
        ...
        <p><u><b>Select Gender:</b></p>
         ...
         <input type="date" name="txt_date"  value=<?= $appointments->Date; ?>> <br><br>
         <label for="tm">Time</label>
         <input type="time" name="txt_time" value=<?= $appointments->Time; ?>><br><br>
         <label for="logo"><b>Enter Disease Type Below:</b></label><br>
         <textarea name="txt_disease" id="txta" width="20px" height="30px" value=<?= $appointments->Disease; ?> > ..</textarea><br><br>

         <span><u><b>Select Doctor:</b></span>
            <select name="doc" id="doc" value=<?= $appointments->Doctor; ?>>
    <?php foreach($dataRecords as $doctors): ?>      <!--- the loop generates a row for each record --->
